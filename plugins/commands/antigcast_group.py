...
def _sweep_antigcast_cooldowns() -> int:
    """
    FIX MEMORY LEAK: _user_cooldown dan _group_cooldown menyimpan expiry
    timestamp tapi tidak pernah membuang entry yang sudah lewat expiry-nya —
    _user_cooldown terutama keyed per user_id GLOBAL (lintas semua grup),
    jadi makin banyak user unik yang pernah pakai /antigcast, makin numpuk
    selamanya. Dipanggil berkala oleh janitor pusat
    (plugins/filters/antispam.py / start_ram_cache_janitor).
    """
    now = time.monotonic()
    removed = 0
    for key in [k for k, exp in _group_cooldown.items() if now >= exp]:
        _group_cooldown.pop(key, None)
        removed += 1
    for key in [k for k, exp in _user_cooldown.items() if now >= exp]:
        _user_cooldown.pop(key, None)
        removed += 1
    return removed
...
async def antigcast_dm_handler(client: Client, message: Message):
    ...
    # ── Cek deeplink parameter ────────────────────────────────────────────────
    # /start pelanggaranku → arahkan ke handler riwayat pelanggaran user
    cmd_args = message.command[1:] if message.command else []
    if cmd_args and cmd_args[0] == "pelanggaranku":
        try:
            from plugins.commands.pelanggaranku import handle_dm_pelanggaranku
            await handle_dm_pelanggaranku(client, message)
        except Exception as e:
            print(f"[antigcast_dm] pelanggaranku error: {e}")
        return
    ...
    # Set ulang command menu DM (tombol "Menu" pojok kiri-bawah) sesuai
    # peran user saat ini (member / admin grup / owner bot). Dijalankan
    # sebagai task terpisah agar tidak menunda balasan /start di atas.
    try:
        from core.dm_menu import set_dm_menu_for_user
        asyncio.create_task(set_dm_menu_for_user(client, uid))
    except Exception as e:
        print(f"[antigcast_dm] ⚠️  Gagal jadwalkan set_dm_menu_for_user: {e}")
