...
# ─────────────────────────────────────────────────────────────────────────────
#  GUARD race condition: promosi NS sedang berjalan
# ─────────────────────────────────────────────────────────────────────────────
# Saat _ns_do_reset_impl memanggil promote_chat_member(), Telegram LANGSUNG
# mengirim ChatMemberUpdated — sebelum ns_set_current_admins() dipanggil.
# ns_watch_forced_admin() yang menerima event itu akan mengecek
# ns_get_current_admins() dan TIDAK menemukan uid (karena ns_set_current_admins
# belum jalan) → salah klasifikasi sebagai "admin paksa dari luar" →
# skor dihapus.
#
# Solusi: _ns_do_reset_impl mendaftarkan setiap uid kandidat ke set ini
# SEBELUM promote_chat_member pertama dipanggil, dan menghapusnya setelah
# ns_set_current_admins selesai. ns_watch_forced_admin membaca set ini
# sebagai short-circuit guard sebelum menyentuh DB sama sekali.
_pending_ns_promotions: set[tuple[int, int]] = set()
    ...
    try:
        ...
        if result.deleted_count:
            ...
            try:
                ...
                invalidate_antispam_vip_cache(chat_id, user_id)
            except Exception:
                pass
...
async def _ns_do_reset_impl(client, chat_id: int):
    ...
    try:
        ...
        if top:
            ...
            # Guard race condition: daftarkan SEMUA kandidat ke
            # _pending_ns_promotions SEBELUM promote_chat_member pertama
            # dipanggil. ChatMemberUpdated dapat tiba kapanpun setelah promote
            # (bahkan saat iterasi masih berjalan), dan ns_watch_forced_admin
            # harus sudah melihat mereka di sini agar tidak salah hapus skor.
            for _w in top:
                _pending_ns_promotions.add((chat_id, _w["user_id"]))
        ...
        # Setelah ns_set_current_admins selesai, bersihkan guard race condition.
        # ns_watch_forced_admin kini akan menemukan mereka di DB dan tidak akan
        # salah menghapus skor admin yang baru diangkat.
        for _w in top:
            _pending_ns_promotions.discard((chat_id, _w["user_id"]))
...
async def newscore_bio_sweep_loop(client):
    ...
    Jalankan sekali dari main.py setelah await app.start(), sama
...
async def ns_watch_forced_admin(client: Client, update: ChatMemberUpdated):
    ...
    try:
        ...
        # FIX race condition: jika uid sedang dalam proses promosi oleh
        # _ns_do_reset_impl (didaftarkan ke _pending_ns_promotions sebelum
        # promote_chat_member dipanggil), ChatMemberUpdated ini PASTI dari
        # NewsCore sendiri — short-circuit, jangan sentuh DB sama sekali.
        if (chat_id, user_id) in _pending_ns_promotions:
            return
