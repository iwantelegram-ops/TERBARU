...
async def cmd_unmutemic(client: Client, message: Message):
    ...
    # ── Custom Userbot grup ini sudah stay permanen di VC — perintah paksaan
    #     lewat antrean worker global TIDAK diperlukan (dan berisiko "tabrakan"
    #     — join/scan tambahan di atas yang sudah berjalan terus-menerus di
    #     akun pribadi admin tersebut). Deteksi bio sudah realtime lewat
    #     _on_vc_update selama Custom Userbot aktif — abaikan diam-diam,
    #     sama seperti kondisi skip lain di command ini.
    try:
        from security_os import custom_userbot as _cub
        if await _cub.is_active(cid):
            print(f"[UnmuteMic] grup={cid}: Custom Userbot aktif (stay permanen) — abaikan, tidak perlu antri.")
            return
    except Exception as _e:
        print(f"[UnmuteMic] Gagal cek Custom Userbot grup={cid}: {_e}")
